...
def make_superpixels():
    with h5py.File('./test_data.h5', 'a') as f:
        bd = f['boundaries'][:]
        ws = stacked_watershed(bd, threshold=.1, sigma_seeds=2.)[0]
        f.create_dataset('watershed', data=ws, compression='gzip')
...
def compute_graph_and_weights(path, return_edge_sizes=False):
    ...
    if return_edge_sizes:
        return graph, feats, edge_sizes, z_edges
    else:
        return graph, feats
...
def run_mc(merge_boutons, beta=.7):
    path = './test_data.h5'
    graph, feats, sizes, z_edges = compute_graph_and_weights(path, return_edge_sizes=True)
...
if __name__ == '__main__':
    ...
    # seg = run_mc(True, beta=.8)
    seg = run_lmc(True, beta=.8)
